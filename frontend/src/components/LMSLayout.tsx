...
export default function LMSLayout({ children, navItems, title, subtitle }: LMSLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
      ...
      >
        {/* Logo/Brand Section - equally proportioned to text */}
        <div className={`flex items-stretch justify-between border-b border-gray-100 bg-gradient-to-r from-primary-600 to-primary-700 ${sidebarOpen ? 'min-h-24 px-5 py-4' : 'min-h-20 px-4 py-4'}`}>
          <div className="flex items-center gap-3 min-w-0 flex-1">
            <div className="flex-shrink-0 w-10 h-10 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center shadow-lg">
              <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
              </svg>
            </div>
            {sidebarOpen && (
              <div className="animate-fade-in flex flex-col justify-center gap-0.5 min-w-0 flex-1">
                <h1 className="text-sm font-bold text-white leading-tight">Wiwik's Smart Classroom</h1>
                <p className="text-[11px] text-primary-100 leading-tight">Analytical AI Flipped Learning System</p>
              </div>
            )}
          </div>
          <button
            onClick={() => setSidebarOpen(!sidebarOpen)}
            className="flex-shrink-0 p-2 rounded-lg hover:bg-white/20 transition-colors text-white self-center"
