...
function getModelName(): string {
  ...
  // 
  // Rate Limits Comparison (approximate, varies by API tier):
  // - gemini-2.0-flash-lite: Highest RPM/TPM (lightweight, fastest, best for high traffic) ⭐ DEFAULT
  // - gemini-3-flash-preview: High RPM/TPM (preview, may change)
  // - gemini-2.5-flash: High RPM/TPM (stable, good alternative)
  // - gemini-2.5-pro: Lower RPM/TPM but more powerful responses
  // - gemini-3-pro-preview: Lower RPM/TPM but most powerful
  //
  // For high traffic production: 
  // - gemini-2.0-flash-lite: Maximum speed and rate limits (current default)
  // - gemini-2.5-flash: Balanced performance and stability (alternative)
  // - gemini-3-flash-preview: Latest features (preview)
  // Set GEMINI_MODEL environment variable to override
...
Now respond as the AI tutor:`;
    ...
    // Based on: https://ai.google.dev/gemini-api/docs/models
    // Rate Limits Comparison (approximate, may vary by tier):
    // - gemini-2.0-flash-lite: Highest RPM/TPM (lightweight, fastest, best for high traffic)
    // - gemini-2.5-flash: High RPM/TPM (stable, best price-performance)
    // - gemini-3-flash-preview: Similar or higher than 2.5-flash
    // - gemini-2.5-pro: Lower RPM/TPM but more powerful
    // - gemini-3-pro-preview: Lower RPM/TPM but most powerful
    const modelsToTry = [
    ...
    for (const tryModelName of modelsToTry) {
      try {
        ...
        // As per: https://ai.google.dev/gemini-api/docs/quickstart
        ...
        // Response structure: response.text (string) or response.candidates[0].content.parts[0].text
        let text: string;
